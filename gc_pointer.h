...
public:
    ...
    static int refContainerSize() { return static_cast<int>(refContainer.size()); }
...
T *Pointer<T, size>::operator=(T *t) {
    ...
    // LAB: Smart Pointer Project Lab
    typename std::list<PtrDetails<T> >::iterator lp = findPtrInfo(addr);
    lp->refcount--;
    if (lp->refcount == 0) collect();

    typename std::list<PtrDetails<T> >::iterator rp = findPtrInfo(t);
    if (rp == refContainer.end()) {
        PtrDetails<T> newPtr(t, size);
        refContainer.push_back(newPtr);
    } else
        rp->refcount++;
...
Pointer<T, size> &Pointer<T, size>::operator=(Pointer &rv) {
    ...
    // the new address.
    typename std::list<PtrDetails<T> >::iterator rp = findPtrInfo(rv.addr);
    // increment ref count
    rp->refcount++;
    // store the address.
    addr = rv.addr;
